...
<body>
    ...
    <div class="container">
        ...
        <div class="gallery">
            <?php
            $stmt = $conn->prepare("SELECT * FROM image2 WHERE cname = ? AND vehicle_num = ? AND date_time = ? ORDER BY id DESC");
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    for ($i = 1; $i <= 5; $i++) {
                        if (!empty($row['image' . $i])) {
                            $imagePaths[] = 'uploads/' . $row['image' . $i];
                        }
                    }
                ...
                if ($colorsMatch && $textsMatch) {
                    echo '<button onclick="note()">DOWNLOAD RECIPT</button>';
                    echo '<button onclick="note1()">BACK TO LOGIN</button>';
                } else {
...
function note1() {
            // Redirect to the home page (change 'home.html' to your actual home page URL)
      ...
      function note() {
                ...
                .then(data => {
                    if (data === 'Success') {
                        alert('Your Request Was submitted, We will contact you shortly');
                        location.href = `invoice2.php?name=${encodeURIComponent(name)}&vehicle_num=${encodeURIComponent(vehicle_num)}`;
                    } else {
                        alert('Error: ' + data);
                    }
                })
