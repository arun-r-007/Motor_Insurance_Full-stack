...
if (isset($_FILES['vehicleImage1']) && isset($_FILES['vehicleImage2']) && isset($_FILES['vehicleImage3']) && isset($_FILES['vehicleImage4']) && isset($_FILES['vehicleImage5'])) {
        ...
        if ($error === 0) {
            if ($img_size > 2097152) {                                         //2mb
                ...
                if (in_array($img_ex_lc, $allowed_exs)) {
                    ...
                    $img_upload_path = 'uploads1/' . $new_img_name;
                    move_uploaded_file($tmp_name, $img_upload_path);
                    $images[] = $new_img_name;
                } else {
                    $em = "You can't upload files of this type";
                    header("Location: 1.php?error=$em");         /////////////
                    exit();
                }
            }
        } else {
    ...
    $sql = "INSERT INTO image3 (cname, vehicle_num, image1, image2, image3, image4, image5,date_time)
            VALUES ('$name', '$vehicle_num','$images[0]', '$images[1]', '$images[2]', '$images[3]', '$images[4]','$date')";

        if (mysqli_query($conn, $sql)) {
            // Redirect to view.php with parameters
            header("Location: view1.php?name=" . urlencode($name) . "&vehicle_num=" . urlencode($vehicle_num)."&date=".urldecode($date));
        } else {
